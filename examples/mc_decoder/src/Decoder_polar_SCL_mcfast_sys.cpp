...
: Decoder_SIHO<B,R>(K, N),
  ...
  dup_count        (L, 0),
  bit_flips        (4 * L),
  bit_flips_r1	   ((L - 1) * L), // bits indices to be flipped * L paths
  is_even          (L),
  best_path        (0),
  n_active_paths   (1),
...
::flip_bits_r1(const int old_path, const int new_path, const int dup, const int off_s, const int n_elmts, const int bits_num)
{
	...
	case 64:
		...
		case 225:// e + a0 + a1 + a16 
			...
			break;
		case 226:// e + a32 
			s[new_path][off_s + bit_flips_r1[bits_num * old_path +32]] = s[old_path][off_s + bit_flips_r1[bits_num * old_path +32]] ? 0 : b;
			break;
		case 227:// e + a33 
			s[new_path][off_s + bit_flips_r1[bits_num * old_path +33]] = s[old_path][off_s + bit_flips_r1[bits_num * old_path +33]] ? 0 : b;
			break;
		case 228:// e + a34 
			s[new_path][off_s + bit_flips_r1[bits_num * old_path +34]] = s[old_path][off_s + bit_flips_r1[bits_num * old_path +34]] ? 0 : b;
			break;
		case 229:// e + a35 
			s[new_path][off_s + bit_flips_r1[bits_num * old_path +35]] = s[old_path][off_s + bit_flips_r1[bits_num * old_path +35]] ? 0 : b;
			break;
		case 230:// e + a36 
			s[new_path][off_s + bit_flips_r1[bits_num * old_path +36]] = s[old_path][off_s + bit_flips_r1[bits_num * old_path +36]] ? 0 : b;
			break;
		case 231:// e + a37 
			s[new_path][off_s + bit_flips_r1[bits_num * old_path +37]] = s[old_path][off_s + bit_flips_r1[bits_num * old_path +37]] ? 0 : b;
			break;
		case 232:// e + a38 
			s[new_path][off_s + bit_flips_r1[bits_num * old_path +38]] = s[old_path][off_s + bit_flips_r1[bits_num * old_path +38]] ? 0 : b;
			break;
		case 233:// e + a39 
			s[new_path][off_s + bit_flips_r1[bits_num * old_path +39]] = s[old_path][off_s + bit_flips_r1[bits_num * old_path +39]] ? 0 : b;
			break;
		case 234:// e + a40 
			s[new_path][off_s + bit_flips_r1[bits_num * old_path +40]] = s[old_path][off_s + bit_flips_r1[bits_num * old_path +40]] ? 0 : b;
			break;
		case 235:// e + a41 
			s[new_path][off_s + bit_flips_r1[bits_num * old_path +41]] = s[old_path][off_s + bit_flips_r1[bits_num * old_path +41]] ? 0 : b;
			break;
		case 236:// e + a42 
			s[new_path][off_s + bit_flips_r1[bits_num * old_path +42]] = s[old_path][off_s + bit_flips_r1[bits_num * old_path +42]] ? 0 : b;
			break;
		case 237:// e + a43 
			s[new_path][off_s + bit_flips_r1[bits_num * old_path +43]] = s[old_path][off_s + bit_flips_r1[bits_num * old_path +43]] ? 0 : b;
			break;
		case 238:// e + a44 
			s[new_path][off_s + bit_flips_r1[bits_num * old_path +44]] = s[old_path][off_s + bit_flips_r1[bits_num * old_path +44]] ? 0 : b;
			break;
		case 239:// e + a45 
			s[new_path][off_s + bit_flips_r1[bits_num * old_path +45]] = s[old_path][off_s + bit_flips_r1[bits_num * old_path +45]] ? 0 : b;
			break;
		case 240:// e + a46 
			s[new_path][off_s + bit_flips_r1[bits_num * old_path +46]] = s[old_path][off_s + bit_flips_r1[bits_num * old_path +46]] ? 0 : b;
			break;
		case 241:// e + a47 
			s[new_path][off_s + bit_flips_r1[bits_num * old_path +47]] = s[old_path][off_s + bit_flips_r1[bits_num * old_path +47]] ? 0 : b;
			break;
		case 242:// e + a48 
			s[new_path][off_s + bit_flips_r1[bits_num * old_path +48]] = s[old_path][off_s + bit_flips_r1[bits_num * old_path +48]] ? 0 : b;
			break;
		case 243:// e + a49 
			s[new_path][off_s + bit_flips_r1[bits_num * old_path +49]] = s[old_path][off_s + bit_flips_r1[bits_num * old_path +49]] ? 0 : b;
			break;
		case 244:// e + a50 
			s[new_path][off_s + bit_flips_r1[bits_num * old_path +50]] = s[old_path][off_s + bit_flips_r1[bits_num * old_path +50]] ? 0 : b;
			break;
		case 245:// e + a51 
			s[new_path][off_s + bit_flips_r1[bits_num * old_path +51]] = s[old_path][off_s + bit_flips_r1[bits_num * old_path +51]] ? 0 : b;
			break;
		case 246:// e + a52 
			s[new_path][off_s + bit_flips_r1[bits_num * old_path +52]] = s[old_path][off_s + bit_flips_r1[bits_num * old_path +52]] ? 0 : b;
			break;
		case 247:// e + a53 
			s[new_path][off_s + bit_flips_r1[bits_num * old_path +53]] = s[old_path][off_s + bit_flips_r1[bits_num * old_path +53]] ? 0 : b;
			break;
		case 248:// e + a54 
			s[new_path][off_s + bit_flips_r1[bits_num * old_path +54]] = s[old_path][off_s + bit_flips_r1[bits_num * old_path +54]] ? 0 : b;
			break;
		case 249:// e + a55 
			s[new_path][off_s + bit_flips_r1[bits_num * old_path +55]] = s[old_path][off_s + bit_flips_r1[bits_num * old_path +55]] ? 0 : b;
			break;
		case 250:// e + a56 
			s[new_path][off_s + bit_flips_r1[bits_num * old_path +56]] = s[old_path][off_s + bit_flips_r1[bits_num * old_path +56]] ? 0 : b;
			break;
		case 251:// e + a57 
			s[new_path][off_s + bit_flips_r1[bits_num * old_path +57]] = s[old_path][off_s + bit_flips_r1[bits_num * old_path +57]] ? 0 : b;
			break;
		case 252:// e + a58 
			s[new_path][off_s + bit_flips_r1[bits_num * old_path +58]] = s[old_path][off_s + bit_flips_r1[bits_num * old_path +58]] ? 0 : b;
			break;
		case 253:// e + a59 
			s[new_path][off_s + bit_flips_r1[bits_num * old_path +59]] = s[old_path][off_s + bit_flips_r1[bits_num * old_path +59]] ? 0 : b;
			break;
		case 254:// e + a60 
			s[new_path][off_s + bit_flips_r1[bits_num * old_path +60]] = s[old_path][off_s + bit_flips_r1[bits_num * old_path +60]] ? 0 : b;
			break;
		case 255:// e + a61 
			s[new_path][off_s + bit_flips_r1[bits_num * old_path +61]] = s[old_path][off_s + bit_flips_r1[bits_num * old_path +61]] ? 0 : b;
			break;
		case 256:// e + a62 
			s[new_path][off_s + bit_flips_r1[bits_num * old_path +62]] = s[old_path][off_s + bit_flips_r1[bits_num * old_path +62]] ? 0 : b;
			break;
...
::_update_r1_vec(const int path, const int array, const int n_elmts, const int bits_num, const int off_l)
{
	...
	{
		bit_flips_r1[bits_num * path + i] = best_idx[i];
		pen[i] = sat_m<R>(std::abs(l[array][off_l + bit_flips_r1[bits_num * path + i]]));
	} 
