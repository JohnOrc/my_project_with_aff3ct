  ...
  l_tmp            (N)
{
	...
	std::cout << "this is a test, and it succeeded" << std::endl;
	...
	else
	{
		...
		if (n_elmts == 2)
		{
			for (auto i = 0; i < n_active_paths; i++)
			{
				...
				const auto pen0 = sat_m<R>(std::abs(l[array][off_l + bit_flips[2 * path +0]])); // a0
				const auto pen1 = sat_m<R>(std::abs(l[array][off_l + bit_flips[2 * path +1]])); // a1
